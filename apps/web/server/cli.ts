...
import { addHelloFreshImageRepairJob, addHelloFreshSyncJob, getQueues, initializeQueues, closeAllQueues } from "@norish/queue";
...
import { getOrCreateManyTags } from "@norish/db/repositories/tags";
...
async function runSyncTags() {
  log.info("[CLI-Tags] Starting tag synchronization and categorization from tags.json...");
  try {
    const tagsFilePath = path.resolve(process.cwd(), "tags.json");
    if (!fs.existsSync(tagsFilePath)) {
      throw new Error(`File not found: ${tagsFilePath}`);
    }
    const rawData = fs.readFileSync(tagsFilePath, "utf8");
    const tags: string[] = JSON.parse(rawData);
    if (!Array.isArray(tags)) {
      throw new Error("Invalid tags.json format. Expected an array of strings.");
    }
    log.info({ count: tags.length }, "[CLI-Tags] Syncing tags...");
    const results = await getOrCreateManyTags(tags);
    log.info({ synced: results.length }, "[CLI-Tags] Tags synced successfully.");
  } catch (error: any) {
    log.error({ err: error.message }, "[CLI-Tags] Failed to sync tags");
    process.exit(1);
  } finally {
...
async function runCategorizeTags() {
  log.info("[CLI-Categorize] Scanning database for uncategorized tags...");
  try {
    const result = await db.execute(sql`
      WITH updates AS (
        UPDATE tags 
        SET category = CASE 
          WHEN name LIKE 'Allergen:%' THEN 'allergen'
          WHEN name LIKE 'Cuisine:%' THEN 'cuisine'
          WHEN name LIKE 'Utensil:%' THEN 'utensil'
          WHEN name LIKE 'Difficulty:%' THEN 'difficulty'
          ELSE 'dietary'
        END
        WHERE category = 'dietary' 
        AND (name LIKE 'Allergen:%' OR name LIKE 'Cuisine:%' OR name LIKE 'Utensil:%' OR name LIKE 'Difficulty:%')
        RETURNING id
      )
      SELECT count(*) from updates;
    `);
    log.info({ updated: result.rows[0]?.count || 0 }, "[CLI-Categorize] Database tags categorized.");
  } catch (error: any) {
    log.error({ err: error.message }, "[CLI-Categorize] Failed to categorize tags");
    process.exit(1);
  } finally {
    setTimeout(() => process.exit(0), 1000);
  }
...
async function runHelloFreshImageRepair() {
  log.info("[CLI-Repair] Enqueueing HelloFresh image repair job...");
  initializeQueues();
  const queues = getQueues();
  try {
    const result = await addHelloFreshImageRepairJob(queues.hellofreshImageRepair, {});
    log.info(`[CLI-Repair] Job status: ${result.status}. Job ID: ${result.job?.id || "N/A"}`);
  } catch (error) {
    log.error({ err: error }, "[CLI-Repair] Failed to enqueue job");
    process.exit(1);
  } finally {
    await closeAllQueues();
    setTimeout(() => process.exit(0), 1000);
...
async function main() {
  ...
  if (command === "hf-sync-tags") return await runSyncTags();
  if (command === "hf-categorize-tags") return await runCategorizeTags();
  if (command === "hf-repair-images") return await runHelloFreshImageRepair();
  ...
  log.error("Unknown command. Available: hf-sync, hf-repair-images, hf-clean, hf-import-file, reset-password, db-truncate, enable-registration, make-admin, delete-user");
  process.exit(1);
}
