...
export class HelloFreshClient {
  ...
  async getRecipe(
    ...
    const json = (await response.json()) as { data: HelloFreshRecipeItem } | HelloFreshRecipeItem;

    // The single-recipe endpoint wraps the payload in a `data` key: { data: { ...recipe } }
    if ("data" in json && json.data && typeof json.data === "object") {
      return json.data;
    }

    return json as HelloFreshRecipeItem;
  }
}
